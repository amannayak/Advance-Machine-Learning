...
Calculating true result 

```{r, echo=FALSE, warning=FALSE , message=FALSE}

dag  = bnlearn::model2network(string = "[A][S][T|A][L|S][B|S][D|B:E][E|T:L][X|E]")
bnlearn::graphviz.plot(dag, main = "True Asia Data BN" 
                       #,  layout = "neato",
                       ,highlight = list(nodes = "S" ,  col = "tomato", 
                                         fill = "orange") )

trueFit = bn.fit(x = dag , data = train ,  method = "bayes")




fnPredict = function(trainData , testData , bnObject , predNode = "S" , Method = "Bayes"){
  
  #fit bn model to training Data 
  modelFit = bnlearn::bn.fit(x = bnObject , data =trainData , method = Method)
  ...
  # and querygrain() to perform posterior inference via belief propagation.  
  ...
  for(i in 1:nrow(testData))
  {
    ...
    testDataRow = as.vector(unname(unlist(test[i, -2])))
    ...
    prediction[i] = ifelse( queryGrain$S[1] < queryGrain$S[2] ,  "yes" , "no")
